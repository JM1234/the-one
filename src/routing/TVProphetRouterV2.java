...
//		tryOtherMessages();	
	...
	public boolean shouldSendFirst(Connection c){
		...
		if (m1.getCreationTime() <= m2.getCreationTime() && weight1<=weight2){ //evaluate whose message is more urgent (with respect to time)
			return true;
		}
		return false;
	}
...
//		if (!buffer.isEmpty()){
		...
		Tuple<Message, Connection> t = tryMessagesForConnected(sortByWeight(getMessagesForConnected()));
		if (t!=null){
			return t.getValue();
		}
//		}
