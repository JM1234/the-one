...
//					System.out.println("otherStatus: "+otherStatus + " OtherAck: "+otherAck);
					...
					if (!otherBuffermap.isEmpty()){ //if othernode has chunks
						...
						if (availableNeighbors.containsKey(msg.getFrom())){
							System.out.println("EVALUATING REQUEST");
							evaluateRequest(host, msg, isFirst); //evaluate what we can get based on latest updates
							System.out.println();
						}
						else if (isInterested(otherAck,  otherBuffermap)){
							sendInterested(host, msg.getFrom());
						}
					}
...
//	}
	...
	private void sendInterested(DTNHost host, DTNHost to) {
		System.out.println(host + " sending interested to " + to);
		String id = APP_TYPE + ":interested-" +	 SimClock.getIntTime();
	...
	protected void sendChunk(StreamChunk chunk, DTNHost host, DTNHost to) {
		String id = APP_TYPE + ":chunk-" + chunk.getChunkID()+  " " + chunk.getCreationTime();
	...
	private void sendRequest(DTNHost host, DTNHost to, long chunkNeeded){
		String id = APP_TYPE + ":request" + "-" + chunkNeeded  + "-" + host.getAddress();
...
//		System.out.println("HERE");
	...
	public void sendResponse(DTNHost host, DTNHost to){
		String id = APP_TYPE + ":UNCHOKE-" + SimClock.getIntTime();
		
		Message m = new Message(host, to, id, SIMPLE_MSG_SIZE);		
		m.addProperty("type", APP_TYPE);
		m.setAppID(APP_ID);
		m.addProperty("msg_type", UNCHOKE);
		m.addProperty(TVProphetRouterV2.MESSAGE_WEIGHT, 1);
		host.createNewMessage(m);
	}
	...
	private void checkExpiredRequest(DTNHost host){
		double curTime = SimClock.getTime();
